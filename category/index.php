...
$str_out_category_name="SELECT * FROM `category` WHERE id='$id_category'";
...
<head>
	<meta charset="utf-8">
	<title>Товары <?php echo "$out_name[cat_name]"; ?>  интернет-магазин Ножницы</title>
...
<div class="breadcrumbs-width">
		<ul class="breadcrumb">
			...
			<li class="breadcrumb-item active_brdcrmb"><?php echo "$out_name[cat_name]"; ?></li>
		</ul>
		</div>
	<div class="top_section">
		...
		<h1><?php echo "$out_name[cat_name]"; ?></h1>
		...
		<nav>
			<ul>
				<?php 
									...
									$str_out_section="SELECT * FROM `sections` WHERE category_id='$id_category'";
...
$str_out_brands="SELECT count( `id` ) AS `post_count`, `brand` FROM `goods` WHERE category='$id_category' GROUP BY `brand` ORDER BY `brand`";
...
<div class='product on_main'>
	...
	elseif ($clear_filters) {
		...
		$str_out_goods="SELECT * FROM `goods` WHERE category='$id_category' AND id > 5 LIMIT 15";
